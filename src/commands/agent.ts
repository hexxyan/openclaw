...
async function agentCommandInternal(
  ...
  try {
    ...
    if (opts.deliver === true || opts.lane === AGENT_LANE_SUBAGENT) {
      ...
      if (targetChannel && targetTo) {
        ...
        if (plugin?.createOutboundDispatcher) {
          const channelBridge = plugin.createOutboundDispatcher({
            cfg,
            agentId: sessionAgentId,
            runtime,
            to: targetTo,
          ...
          const bridgeReplyOptions = channelBridge.replyOptions ?? {};
    ...
    const bridgeReplyOptions = streamingBridge?.replyOptions;
