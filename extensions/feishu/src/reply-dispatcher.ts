...
function resolveCardHeader(
  agentId: string,
  identity: any, // Use any to allow 'theme' which is feishu-specific metadata on identity
): CardHeaderConfig | undefined {
  const name = identity?.name?.trim();
  // Strictly hide the header if no custom name is provided to keep the UI clean.
  if (!name) {
    return undefined;
  }
...
function resolveCardNote(
  ...
  stats?: { startTime: number; charCount: number },
): string {
  const name = identity?.name?.trim() || agentId;
  // Truncate name/ID if it's too long, showing only first 6 hex chars (total 12 chars).
  const shortName = name.length > 12 ? name.slice(0, 12) : name;
  const parts: string[] = [`Agent: ${shortName}`];

  if (stats && stats.startTime > 0) {
    const elapsedS = (Date.now() - stats.startTime) / 1000;
    if (elapsedS > 0.1) {
      parts.push(`耗时: ${elapsedS.toFixed(1)}s`);
      // Rough approximation of tokens/s (1 token ≈ 1.5 - 2 chars, so chars/2 is safe)
      const tps = stats.charCount / 2 / elapsedS;
      if (tps > 0) {
        parts.push(`Token 速度: ${tps.toFixed(1)} t/s`);
      }
    }
  }
  ...
  // User requested to remove Provider info for a cleaner look.
  return parts.join(" | ");
}
...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  const queueStreamingUpdate = (
    ...
    if (nextText) {
      ...
      totalCharsDelivered += nextText.length;
    ...
    partialUpdateQueue = partialUpdateQueue.then(async () => {
      ...
      if (streaming?.isActive()) {
        ...
        const currentNote = resolveCardNote(
          agentId,
          identity,
          prefixContext.prefixContext,
          startTime > 0 ? { startTime, charCount: totalCharsDelivered } : undefined,
        );
        await streaming.replaceContent(fullContent, { note: currentNote });
      }
    });
  };
    ...
    core.channel.reply.createReplyDispatcherWithTyping({
      ...
      deliver: async (payload: ReplyPayload, info) => {
        ...
        if (shouldDeliverText) {
          ...
          if (info?.kind === "block") {
            const blockStreamingEnabled = (account.config as any)?.blockStreaming !== false;
